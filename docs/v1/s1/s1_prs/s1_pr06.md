...
- json write failures should surface as `AgencyError{code: <appropriate>}`:
  - use `E_SCRIPT_FAILED` **never**
  - use `E_RUN_DIR_EXISTS` for mkdir exists
  - use `E_IO` only if you already have a generic io code; otherwise introduce:
    - `E_META_WRITE_FAILED` (optional). if you want minimal codes, reuse `E_SCRIPT_FAILED` is wrong; prefer a dedicated code.

**recommendation:** introduce `E_META_WRITE_FAILED` only if you don’t already have a generic io/write code.
...
- create `runs/<run_id>/` such that it fails if it already exists:
  - use `os.Mkdir(runDir, 0o755)` (or 0o700; pick one and be consistent across repo)
  - if exists: return `AgencyError` with `code=E_RUN_DIR_EXISTS`, include `run_dir` in details
- create `logs/` inside run dir (`os.MkdirAll`) after run dir exists
...
constraints:
	...
	•	if you discover naming mismatches (different package names than assumed), adapt to repo conventions; do not invent a new architecture.
