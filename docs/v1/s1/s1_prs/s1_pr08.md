...
- implement `agency attach <run_id>` (no resume behavior in this PR):
  ...
  - attach to tmux session:
    - preferred: use the `tmux_session_name` in meta if present
    - fallback: derive `agency:<run_id>` only if `tmux_session_name` absent (but still check existence)
  - invocation:
...
- attach error cases:
  ...
  - if run_id exists elsewhere (optional; if implemented): `E_RUN_REPO_MISMATCH`
...
- `E_NO_REPO`
- `E_RUN_NOT_FOUND`
- `E_RUN_REPO_MISMATCH` (only if already defined in earlier specs; if not, omit in v1 and treat as E_RUN_NOT_FOUND)
...
- setup step succeeded (no `flags.setup_failed`)
...
- `TestBuildRunnerShell_IncludesEscapedWorktreeAndRunnerCmd` (if helper tests not already present):
...
- `TestStartTmux_UpdatesMetaOnSuccess`:
  - can be a “thin unit test” if tmux invocation is wrapped behind an interface
...
go test ./...
	...
	•	create detached session agency:<run_id>
