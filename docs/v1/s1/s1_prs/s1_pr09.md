...
- default values:
  - `--title` default: `untitled-<shortid>` (where `<shortid>` is derived from the generated run_id; use the same rule used in earlier PRs)
...
- run the already-implemented internal pipeline:
  - must use the pipeline function introduced in pr-04 (e.g. `RunPipeline(...)`)
...
- print (stdout) the following lines (order fixed):

1. `run_id: <run_id>`
2. `title: <title>`
3. `runner: <runner_name>`
4. `parent: <parent_branch>`
5. `branch: <branch>`
6. `worktree: <worktree_path>`
7. `tmux: agency:<run_id>`
...
- print (stderr):
  - `error: <CODE>: <message>`
- if a run_id exists by the time it failed, also print:
  - `run_id: <run_id>`
  - `worktree: <worktree_path>` (if created)
  - `setup_log: <absolute_path_to_setup.log>` (only if setup step started/created log)
- exit non-zero.
...
automated (required)
	...
	•	--title sets title
	•	default title is untitled-<shortid> when omitted
