...
rules:
	...
	•	if --force true: overwrite agency.json (atomic write required via WriteFileAtomic)
...
create stub scripts (never overwrite)
	...
	•	init always creates stubs at these canonical paths, regardless of any preexisting agency.json script paths
...
.gitignore behavior
	...
	•	treat any existing line equal to .agency/ or .agency as present (exact match)
...
keep output stable and parseable (stdout):
	...
	•	repo_root: <path>
	•	agency_json: created|overwritten
	•	scripts_created: <comma-separated list or 'none'>
	•	gitignore: updated|unchanged|skipped
	•	if --no-gitignore: include warning: gitignore_skipped

implementation note:
	•	consider a shared key/value output helper for init/doctor to keep formatting stable
...
integration-style tests using a temp directory:
	...
	•	and: .gitignore with ".agency" (no slash) is treated as present
...
notes:
	...
	•	assert only that owner executable bit is set (avoid strict mode equality across platforms)
...
deliverables:
	...
	•	tests passing
	•	no scope creep
	6.	gitignore newline handling

	•	given: .gitignore exists without trailing newline
	•	when: init
	•	then: .gitignore ends with newline
