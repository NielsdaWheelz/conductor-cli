...
- compute repo identity + origin status:
  ...
  - determine:
    ...
    - `github_flow_available` (bool) = `repo_key` has prefix `github:`
...
- runner resolution + existence check:
  ...
  - **v1 constraint:** runner command must be a single executable token (no args). trim whitespace; if empty or contains whitespace, treat as invalid config (`E_INVALID_AGENCY_JSON`) with message “runner commands must be a single executable (no args); use a wrapper script”
  - relative runner paths are resolved relative to repo root
- script checks (no execution):
  ...
  - symlink scripts are allowed; follow symlink to check executable bit
- output contract:
...
rules:
- if command not found (exec error): fail with *_NOT_INSTALLED code
- else if exit code != 0: fail with *_NOT_INSTALLED or *_NOT_AUTHENTICATED depending on command
- capture versions from stdout (trim whitespace). do not parse beyond “a stable line string”.
- for `gh auth status`, ignore stdout/stderr; only exit code matters
...
- symlink scripts are allowed; follow symlink for executable check
...
- always print every key in this order; unknown values are empty strings; booleans are true|false.
- for errors, stderr prints `error_code: E_...` then a message line; optional `hint:` line. keep stdout empty on failure.

### persistence (success only)
only after all checks pass:
1) ensure `${AGENCY_DATA_DIR}/repos/<repo_id>/` exists
2) write/update `repos/<repo_id>/repo.json`
3) write/update `repo_index.json`
...
- maintain `paths` as unique strings, MRU insert at index 0 (per store rules)

**repo.json** (schema v1):
- top-level fields:
  - `schema_version: "1.0"`
  - `repo_key`, `repo_id`
  - `repo_root_last_seen` (repo_root)
  ...
  - `updated_at` (rfc3339, update each success)

atomic writes:
- write to temp file in same directory + rename
- do not write any json files if any check fails
- if any persistence step fails: return `E_PERSIST_FAILED` and do not claim success
- do not create `${AGENCY_DATA_DIR}/repos/<repo_id>` unless persistence will happen
...
- assert:
  ...
  - created_at stable across second run (run twice; created_at unchanged, updated_at updated)
