...
minimum data fields:
	•	for cmd_start: { "cmd": "show", "args": ["--capture", "<id>"] } (args best-effort; ok if not exact order)
...
notes:
	...
	•	print warnings (human output)
	•	optionally include extra fields in data like { "capture_ok": false, "capture_warning": "..." } (allowed; additive)
...
agency show --capture is mutating:
	...
	•	it must emit cmd_start at the beginning of the locked section
	•	it must emit cmd_end at the end (even if capture failed; best-effort)
...
session missing
	...
	•	does not write transcript files (unless you choose to write a placeholder; default is do not)
...
unit tests
	...
	•	malformed sequences
	•	assert:
	•	output contains no \x1b[ sequences
