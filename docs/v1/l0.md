...
core loop:
1. create workspace from a parent branch (requires clean working tree)
...
- no transcript replay on resume (v1)
...
- the durable unit agency manages:
  ...
  - workspace-local artifacts (`.agency/` inside worktree)
...
- runner can always be relaunched via `agency resume`.
...
**scripts**: never prompt, ever. stdin is `/dev/null`. scripts execute outside tmux; the tmux pane is reserved for the runner TUI.
...
all scripts must be:
- **non-interactive**: no prompts; stdin is `/dev/null`
- **idempotent**: safe to re-run
- **bounded**: agency enforces timeouts
...
scripts execute **outside tmux** in a normal subprocess. the tmux pane is reserved for the runner TUI only.

### script execution context

agency creates these directories in the workspace before script execution:
- `<workspace>/.agency/`
- `<workspace>/.agency/out/`
- `<workspace>/.agency/tmp/`

scripts receive environment variables:
- `AGENCY_RUN_ID` — run identifier
- `AGENCY_REPO_ROOT` — absolute path to main repo
- `AGENCY_WORKSPACE_ROOT` — absolute path to worktree
- `AGENCY_TITLE` — human title for this run
- `AGENCY_PARENT_BRANCH` — branch this run is based on
- `AGENCY_BRANCH` — branch name for this run
- `AGENCY_ORIGIN_NAME` — git remote name (typically `origin`)
- `AGENCY_ORIGIN_URL` — git remote url
- `AGENCY_RUNNER` — runner kind (`claude` or `codex`)
- `AGENCY_PR_URL` — PR url (empty if no PR yet)
- `AGENCY_PR_NUMBER` — PR number (empty if no PR yet)
- `AGENCY_DOTAGENCY_DIR` — path to `<workspace>/.agency/`
- `AGENCY_OUTPUT_DIR` — path to `<workspace>/.agency/out/`
- `AGENCY_LOG_DIR` — path to run logs in global store
- `AGENCY_NONINTERACTIVE=1` — always set
- `CI=1` — always set (scripts should behave as in CI)
...
scripts may write structured output to known paths (if present, must follow schema):
...
agency reads these for evidence. if `verify.json` is missing after verify runs, treat as failure with `ok: false`.
...
all durable run state lives here:
...
### workspace-local (`.agency/` inside worktree) — runner-writable

workspace-local artifacts (gitignored):
- `.agency/report.md` — report file, editable by runner
- `.agency/out/` — structured script outputs
- `.agency/tmp/` — scratch space for scripts
...
- attach/detach semantics are tmux-native:
  ...
  - if session missing, `attach` errors and suggests `resume`
...
### canonical path: workspace-local

report lives at `<workspace>/.agency/report.md` — editable by runner.
...
default is `pr_body` mode (workspace-local, synced to PR).

### machine-facing artifacts

stored in global run directory (not workspace):
...
# 3. if runner exited, resume it
agency resume <id>
# relaunches runner in existing workspace, then attaches

# 4. push when ready for review
agency push <id>
# creates PR if missing, syncs report to PR body

# 5. review via github or locally
agency code <id>
# opens the worktree in the ide. runner may still be active
...
- `agency attach <id>` — attach to existing tmux session; error if session missing
- `agency resume <id> [--detached]` — ensure tmux session exists, (re)start runner if not running, attach unless `--detached`
- `agency stop <id>` — signal runner / stop session (does not delete workspace)
- `agency push <id>` — push branch, create/update PR, sync report (idempotent)
- `agency code <id>` — open workspace in IDE
...
- `attach`: attach to existing tmux session. if session does not exist, error with `E_SESSION_NOT_FOUND` and suggest `resume`.
- `resume`: ensure tmux session exists (create if needed), start runner if not currently running, then attach (unless `--detached`). no transcript replay in v1.
...
- agency must never start a run if parent working tree is dirty (`E_PARENT_DIRTY`)
